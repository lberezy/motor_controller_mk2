...
//*****************************************************************************
#ifdef DEBUG
bool I2C_isHandleValid(I2C_Handle i2cHandle)
{
        ...
        const  uint16_t bitTimeLow, const uint16_t bitTimeHigh)
{
    ...
    i2c->I2CPSC = preScalar; // & I2C_I2CPSC_IPSC_BITS;
...
I2C_IntSource_e I2C_getIntSource(I2C_Handle i2cHandle)
{
    ...
    source = (I2C_IntSource_e)(i2c->I2CISRC);

    // Return the i2c int source
    return (source);
}
...
I2C_Status_e I2C_getStatus(I2C_Handle i2cHandle)
{
    ...
    status = (I2C_Status_e)(i2c->I2CSTR);
...
//! \b false.
//*****************************************************************************
bool I2C_isMasterBusy(I2C_Handle i2cHandle)
{
    // Check the arguments.
    //assert(I2C_isHandleValid(i2cHandle));
    ...
    bool status;
    // get the status
    status = (i2c->I2CSTR & I2C_I2CSTR_BB_BITS) >> 12;
...
void I2C_MasterControl(I2C_Handle i2cHandle, I2C_Control_e action, uint16_t bitCount, uint16_t bytes)
{
    ...
    // Send the command.
    // Added by Maya
    i2c->I2CMDR &= ~(I2C_I2CMDR_MST_BIT | I2C_I2CMDR_TRX_BIT | I2C_I2CMDR_STT_BIT | I2C_I2CMDR_STP_BIT); // clear bits 9, 10, 11, 13
...
// Added by Maya
void I2C_resetAll(I2C_Handle i2cHandle)
{
...
void I2C_clearStopConditionDetection(I2C_Handle i2cHandle)
{
    I2C_Obj *i2c = (I2C_Obj *)i2cHandle;
...
bool I2C_isStopConditionDetected(I2C_Handle i2cHandle)
{
	I2C_Obj *i2c = (I2C_Obj *)i2cHandle;

    bool status;
    // get the status
    status = (i2c->I2CSTR  & I2C_I2CSTR_SCD_BITS) >> 5;

    return((bool)status);

/*	// Return the stop condition bit status.
	if(i2c->I2CSTR  & I2C_I2CSTR_SCD_BITS)
	{
		return(true);
	}
	else
...
bool I2C_isAllRegisterReady(I2C_Handle i2cHandle)
{
	I2C_Obj *i2c = (I2C_Obj *)i2cHandle;

    bool status;
    // get the status
    status = (i2c->I2CSTR  & I2C_I2CSTR_ARDY_BITS) >> 2;

    return((bool)status);

/*	// Return the stop condition bit status.
	if(i2c->I2CSTR  & I2C_I2CSTR_ARDY_BITS)
	{
		return(true);
	}
	else
...
bool I2C_isNoAck(I2C_Handle i2cHandle)
{
	I2C_Obj *i2c = (I2C_Obj *)i2cHandle;

    bool status;
    // get the status
    status = (i2c->I2CSTR  & I2C_I2CSTR_NACK_BITS) >> 1;
...
void I2C_clearNoAckBit(I2C_Handle i2cHandle)
{
...
void I2C_setMasterStopBit(I2C_Handle i2cHandle)
{
...
bool I2C_isMasterStopBitSet(I2C_Handle i2cHandle)
{
	I2C_Obj *i2c = (I2C_Obj *)i2cHandle;

    bool status;
    // get the status
    status = (i2c->I2CMDR & I2C_I2CMDR_STP_BIT) >> 11;

    return((bool)status);

/*	// Return the stop condition bit status.
	if(i2c->I2CMDR & I2C_I2CMDR_STP_BIT)
	{
		return(true);
	}
	else
...
void I2C_setRunFree(I2C_Handle i2cHandle)
{
    I2C_Obj *i2c = (I2C_Obj *)i2cHandle;
...
void I2C_clearRunFree(I2C_Handle i2cHandle)
{
    I2C_Obj *i2c = (I2C_Obj *)i2cHandle;
...
extern void I2C_writeData(I2C_Handle i2cHandle, uint16_t addr, uint16_t reg, uint16_t data) {
	...
	i2c->I2CSAR = addr; // set device address to communicate with

	// busy wait on bus-busy
	//while(i2c->I2CSTR & I2C_I2CSTR_BB_BITS);

	i2c->I2CCNT = 2; // write successive device register and register data bytes
	i2c->I2CDXR = reg;
	i2c->I2CDXR = data;
	i2c->I2CMDR = (I2C_I2CMDR_STT_BIT | I2C_I2CMDR_STP_BIT | I2C_I2CMDR_MST_BIT | I2C_I2CMDR_IRS_BIT | I2C_I2CMDR_TRX_BIT | I2C_I2CMDR_FREE_BIT);
